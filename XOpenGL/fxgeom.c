...
// alas....
//
// This is the OpenGL backend: the transformation and clipping code is
// identical to the Glide backend, only the final drawing calls
// (grDrawTriangle/grDrawLine/grDrawPoint) are replaced with OpenGL immediate
// mode calls.
//
// Depth: Glide used W-buffering with depth = oow (1/w). OpenGL has no
// w-buffer, so the oow value is passed as the vertex Z and compared with
// GL_LEQUAL (nearer = larger oow = closer). See mode_change() in fx.c.

#include "pch.h"

// Vertex layout produced by setuprvx(). Same 15-float layout as Glide's
// GrVertex (see the XGLIDE_Decompile Readme):
//   x, y, z | r, g, b | ooz | a | oow | sow, tow, oow | sow, tow, oow (TMU1)
typedef struct
{
	float x, y, z;		// screen space (y up)
	float r, g, b;		// 0..255
	float ooz;
	float a;			// 0..255
	float oow;			// 1/w  (depth)
	float sow, tow;		// TMU0 texture coords (s/t over w)
	float oow0;
	float sow1, tow1;	// TMU1 texture coords (s/t over w)
	float oow1;
} xvertex;
...
static int clipor;
static int clipbuf1[0x200];
static int clipbuf2[0x200];
static int* clipin;
static int* clipout;

static xvertex grvx[0x200];			// Item size: 60 bytes
...
static uint8_t xformed[0x200];

static int splitbuf[0x200];
...
/// <summary>
/// Matrix multiplication + clipping. Works in 1 of 4 modes (XFORM_MODE).
/// </summary>
			...
			if (!v2->clip)
			{
				...
				else
				{
					if (g_state[XST].send & 2)
					{
						v3[9] = *v4 * v17;
						v3[10] = v4[1] * v17;
						*((DWORD *)v3 + 11) = *((DWORD *)v3 + 8);	// oow for TMU0
					}
					if (!(g_state[XST].send & 4))
						goto LABEL_23;
				}
...
// Triangulate a convex polygon (vertex index list a2, a1 entries) into a fan
// of triangles written to a3. Returns the number of indices written.
int splitpoly(int a1, int* a2, DWORD *a3)
{
	...
	v5 = i = 0;
	for (v17 = 1; a1 > v17; ++v17)
	{
		if (grvx[a2[v17]].y > grvx[a2[i]].y)
			i = v17;
	}
	v5 = i - 1;
	if (i - 1 < 0)
...
void flush_reordertables()
{
	...
	if (v1 <= vertices)
	{
		...
		memmove(&grvx[v2], &grvx[vertices - v1], v1 * sizeof(xvertex));
...
// Emit one vertex with all its attributes to OpenGL.
static void draw_vertex(const xvertex* v)
{
	if (g_state[XST].send & 1)
		glColor4f(v->r * (1.0f / 255.0f), v->g * (1.0f / 255.0f), v->b * (1.0f / 255.0f), v->a * (1.0f / 255.0f));

	if (g_state[XST].send & 2)
	{
		// Texture coordinate in the library's "256-unit" space; the texture
		// matrix (set in mode_loadtexture) normalizes it to [0..1].
		glTexCoord4f(v->sow, v->tow, 0.0f, v->oow);
	}
	if (g_state[XST].send & 4)
	{
		if (glMultiTexCoord4fARB)
			glMultiTexCoord4fARB(GL_TEXTURE1_ARB, v->sow1, v->tow1, 0.0f, v->oow);
	}

	// Fog coordinate = w = 1/oow (see mode_change() in fx.c). This keeps the
	// fog curve independent of the depth value passed to glVertex3f.
	if (glFogCoordfEXT && g_state[XST].active.fogtype != X_DISABLE)
		glFogCoordfEXT(v->oow != 0.0f ? 1.0f / v->oow : 0.0f);

	// Depth = oow (Glide used W-buffering; OpenGL has no w-buffer, so the
	// 1/w value is used as Z). Nearer fragments have larger oow, see
	// mode_change() depth setup.
	glVertex3f(v->x, v->y, v->oow);
}

// Set the constant (non-iterated) color used when the combiner does not need
// per-vertex colors.
static void set_constant_color(void)
{
	if (g_state[XST].active.colortext1 == X_WHITE)
		glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
	else
		glColor4f(g_state[XST].active.env[0], g_state[XST].active.env[1],
			g_state[XST].active.env[2], g_state[XST].active.env[3]);
}

static void draw_triangle(const xvertex* a, const xvertex* b, const xvertex* c)
{
...
static void draw_line(const xvertex* a, const xvertex* b)
{
	glBegin(GL_LINES);
	draw_vertex(a);
	draw_vertex(b);
...
static void draw_point(const xvertex* a)
{
	glBegin(GL_POINTS);
	draw_vertex(a);
	glEnd();
...
static void draw_fan(const xvertex* const* vlist, int count)
{
	int j;
...
int flush_drawfx()
{
	...
	// Constant (non-iterated) color, used when the combiner does not need
	// per-vertex colors.
	if (!(g_state[XST].send & 1))
		set_constant_color();
	...
	for (i = corner[0]; i; v0 = v15 + 1)
	{
		...
		if (i > 0)
		{
			// v0 points into the corner array; read i vertex indices
			int* vcur = v0;
			v2 = i;
			do
			{
				...
				else
				{
					...
					if (result >= 3)
					{
						...
						if (g_state[XST].setnew & 1)
						{
							...
							for (j = 0; v10 > j; ++j)
							{
								draw_line(&grvx[v16[v2]], &grvx[v16[j]]);
								v2 = j;
							}
						}
						else
						{
							result = splitpoly(v10, v16, splitbuf);
							if (result > 0)
							{
		...
		else if (g_state[XST].setnew & 1 && i > 2)
		{
			...
			if (i > 0)
			{
				do
				{
					draw_line(&grvx[v0[v2]], &grvx[v0[v8]]);
					v2 = v8++;
				}
				while (i > v8);
			}
		...
		else
		{
			switch (i)
			{
				case 1:
					++g_stats.out_tri;
					draw_point(&grvx[v0[0]]);
					break;
				case 2:
					++g_stats.out_tri;
					draw_line(&grvx[v0[0]], &grvx[v0[1]]);
					break;
				case 3:
					++g_stats.out_tri;
					draw_triangle(&grvx[v0[0]], &grvx[v0[1]], &grvx[v0[2]]);
					break;
				default:
					// Polygons (quads, etc.) - fan triangulation.
					// (The Glide backend could not do this and dropped them.)
					g_stats.out_tri += i - 2;
					for (j = 0; j < i; j++)
						vl[j] = &grvx[v0[j]];
					draw_fan(vl, i);
